...
ProjectEulerModule::ProjectEulerModule()
{
}
...
int ProjectEulerModule::Problem1(){
	clock_t begin = clock();
	int sum = 0;
	...
	clock_t end = clock();
	cout << double(end - begin) / CLOCKS_PER_SEC << " time elapsed" << endl;
	return sum;
}

//By listing the first six prime numbers: 2, 3, 5, 7, 11, and 13, we can see that the 6th prime is 13. What is the 10 001st prime number ?
int ProjectEulerModule::Problem7(){
	clock_t begin = clock();
	int count = 6;
	int number = 13;
	...
	while (count < 10001){
		number++;
		if (MathematicsModule::IsPrime(number)){
...
int ProjectEulerModule::Problem9(){
	int triples[16][3] = {
		{ 3, 4, 5 }, { 6, 12, 13 }, { 8, 15, 17 }, { 7, 24, 25 },
	...
	for (int i = 0; i < 16; i++){
		int a = triples[i][0], b = triples[i][1], c = triples[i][2], sum = 0, k = 1;
		while (sum < 1000){
			sum = (k*a + k*b + k*c);
			if (sum == 1000)
				return a*b*c*pow(k,3);
			k++;
		}
	}
	return 0;
...
long long ProjectEulerModule::Problem10(){
	bool *primes = MathematicsModule::Eratosthenes(2000000);
	long long sum = 0;
	int count = 0;
...
*/
long long ProjectEulerModule::Problem11(){
	int grid[20][20] = {
	...
	for (int i = 0; i < 20; i++){
		for (int j = 0; j < 17; j++){
			long long right = grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3];
			gp = max(gp, right);
		}
	}
	////Down
	for (int i = 0; i < 17; i++){
		for (int j = 0; j < 20; j++){
			long long right = grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j];
			gp = max(gp, right);
		}
	}
	////Diagonal-right
	for (int i = 0; i < 17; i++){
		for (int j = 0; j < 17; j++){
			long long right = grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3];
			gp = max(gp, right);
		}
	}
	//Diagonal-left
...
*/
int ProjectEulerModule::Problem12(){
	int count =1, triNumber = 0, numDivisors = 1;

	do
	{
		triNumber = count*(count + 1) / 2;
		numDivisors = MathematicsModule::GetNumberFactors(triNumber);
		count++;
...
*/
int ProjectEulerModule::Problem14(){
	clock_t begin = clock();
	vector<int> cache;
	int m = 0, result;
	for (int i = 0; i < 1000000; i++){
		int length = collatzLength(i, &cache);
		if (length > m){
			m = length;
			result = i;
		}		
		//cache.insert(cache.end(), length);
	}
	clock_t end = clock();
	cout << double(end - begin) / CLOCKS_PER_SEC << " time elapsed" << endl;
	return result;
}
long long ProjectEulerModule::collatzLength(long long n, vector<int> *cache, int count){
	//cout << n << endl;
	if (n == 0)
		return 0;
	if (n == 1)
		return count;
	//if (find(cache->begin(), cache->end(), n) != cache->end())
	//	return cache->at(n);
	if (n % 2 == 0)
		n = n / 2;
	else
...
*/
int ProjectEulerModule::Problem15(){
	//It's a pascals triangle. Just do 40 choose 20
	return 0;
}

/*215 = 32768 and the sum of its digits is 3 + 2 + 7 + 6 + 8 = 26.
What is the sum of the digits of the number 21000?*/
int ProjectEulerModule::Problem16(){
	return 0;
}
...
int ProjectEulerModule::Problem17(){
	int result=0;
	for (int i = 1; i <= 1000; i++)
		result += cardinalNumberLength(i);
	return result;
	//while (true){
	//	int value;
	//	printf("Enter a number:");
	//	scanf_s("%d", &value);
	//	cout << cardinalNumberLength(value) << endl;
	//}
}
int ProjectEulerModule::cardinalNumberLength(int n){
	if (n == 1 || n == 2 || n == 6 || n == 10)
		return 3;
		...
		return cardinalNumberLength(1) + 8;
	else if (n >= 100)
		return cardinalNumberLength(n / 100) + 7 + (n % 100 > 0 ? 3 + cardinalNumberLength(n % 100) : 0);
	else if (n >= 90)
		return cardinalNumberLength(90) + cardinalNumberLength(n % 10);
	else if (n >= 80)
		return cardinalNumberLength(80) + cardinalNumberLength(n % 10);
	else if (n >= 70)
		return cardinalNumberLength(70) + cardinalNumberLength(n % 10);
	else if (n >= 60)
		return cardinalNumberLength(60) + cardinalNumberLength(n % 10);
	else if (n >= 50)
		return cardinalNumberLength(50) + cardinalNumberLength(n % 10);
	else if (n >= 40)
		return cardinalNumberLength(40) + cardinalNumberLength(n % 10);
	else if (n >= 30)
		return cardinalNumberLength(30) + cardinalNumberLength(n % 10);
	else if (n >= 20)
		return cardinalNumberLength(20) + cardinalNumberLength(n % 10);	
}
...
NOTE: As there are only 16384 routes, it is possible to solve this problem by trying every route. However, Problem 67, is the same challenge with a triangle containing one-hundred rows; it cannot be solved by brute force, and requires a clever method! ;o)s*/
int ProjectEulerModule::Problem18(){
	clock_t begin = clock();
	int tree[] = { 
		              75,
		             95,64,
	...
	int cache[120];
	int result = TreeMaxSum(tree, cache, 120);
	clock_t end = clock();
	cout << double(end - begin) / CLOCKS_PER_SEC << " time elapsed" << endl;
	return result;
}
int ProjectEulerModule::TreeMaxSum(int tree[], int cache[], int treeLength, int root, int row){
 	if (root + row + 1 >= treeLength)
		return tree[root];
	if (cache[root] > 0)
		return cache[root];
	//Get the max sum of the left sub triangle
	int left = TreeMaxSum(tree, cache, treeLength, root + row + 1, row + 1);
	//Get the max sum of the right sub triangle
	int right = TreeMaxSum(tree, cache, treeLength, root + row + 2, row + 1);
	int result = max(tree[root] + left, tree[root] + right);
	cache[root] = result;
	return result;
}
...
int ProjectEulerModule::Problem67(){
	clock_t begin = clock();
	int tree[] = {
		59,
		73,41,
	...
	int totalSum = TreeMaxSum(tree, cache, length);
	clock_t end = clock();
	cout << double(end - begin) / CLOCKS_PER_SEC << " time elapsed" << endl;
	return totalSum;
}
...
int ProjectEulerModule::Problem19(){
	return 0;
}